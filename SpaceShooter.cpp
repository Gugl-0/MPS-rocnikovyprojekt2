...
void WelcomeMessage()
{
	...
	gotoxy(30, y + 6); printf("Press any key to play...");
	gotoxy(26, y + 7); printf("Developed by Carlos Hernandez C. - 2016");
}
...
void GameOverOutOfAmmoMessage()
{
	...
	int b_y = a_y + 4;
	DrawWindowFrame(a_x, a_y, b_x, b_y);
	gotoxy(a_x + 1, a_y + 2); printf("      Out of ammo!!!");
}
void SpecialMessage()
{
...
int main()
{
	...
	for (int i = 0; i < 10; i++)
	{
		Asteroids.push_back(new Asteroid(rand() % 72 + 1, rand() % 4 + 3));
	}
	...
	bullets = maxBullets = 40;

	while (!ss.isDead() && score != 150)
	{
		frameCount++;
		if (_kbhit())
		{
		...
		for (asteroid = Asteroids.begin(); asteroid != Asteroids.end(); ) {
